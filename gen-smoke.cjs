...
// Each paragraph ends with a unique spirit-stone count (101..190) for
// mechanical order/completeness verification in the translated output.
const fs = require("fs");
...
const SENTS = [
  ...
  (n, p, it) => `夜色渐深，山间的兽鸣此起彼伏，${n}却充耳不闻，一心沉浸于吐纳之中。`,
...
const paragraphs = [];
for (let i = 1; i <= 90; i++) paragraphs.push(`〔${i}〕` + paragraph(i));
const text = paragraphs.join("\n\n");

const payload = {
  fileName: `buffy-smoke-seam-${Date.now()}.txt`,
  ...
  smoke: { forceMaxOutputTokens: 6000 },
  chunks: [{ text, gzip: false, originalId: 0, partIndex: 0, partCount: 1 }],
  liveModels: [
...
console.log("chars:", text.length, "| paras:", paragraphs.length, "| file:", payload.fileName);
